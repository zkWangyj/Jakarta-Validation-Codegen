 ...
 * 
 * <p>每个注解对应一个 checkXxx 方法，注解处理器会在标注了 @PreCompile 的方法体中
 * 自动注入对这些方法的调用。</p>
 * 
 * <p>使用示例（编译后自动生成）：</p>
 * <pre>
...
public class ValidationHelper {
    ...
    public static void checkSize(Object value, int min, int max, String message) {
        ...
        int len;
        if (value instanceof String) len = ((String) value).length();
        else if (value instanceof Collection) len = ((Collection<?>) value).size();
        else if (value instanceof Map) len = ((Map<?, ?>) value).size();
        else if (value.getClass().isArray()) len = java.lang.reflect.Array.getLength(value);
        else return;
        if (len < min || len > max) {
            throw new IllegalArgumentException(message);
        }
